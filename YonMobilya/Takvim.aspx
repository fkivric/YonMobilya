<%@ Page Title="" Language="C#" MasterPageFile="~/admin.Master" AutoEventWireup="true" CodeBehind="Takvim.aspx.cs" Inherits="YonMobilya.Takvim" %>
    ...
    $(document).ready(function () {
        ...
        $("#calendar").datepicker({
            beforeShowDay: function(date) {
                ...
                if (ids.length > 0) {
                    // Her ID'yi ayrı ayrı göstermek için span'lere ekliyoruz
                    var displayText = ids.map(id => '<span class="event-count">' + id + '</span>').join(" ");
            ...
            onSelect: function(dateText, inst) {
                var selectedDate = $.datepicker.parseDate("dd.mm.yy", dateText);
                var formattedDate = $.datepicker.formatDate("yy-mm-dd", selectedDate);
                var ids = eventCounts[formattedDate]; // MSSQL'den gelen JSON verisi (birden fazla ID olabilir)

                if (ids && ids.length > 0) {
                    var idList = ids.join(','); // Tüm ID'leri virgülle ayırarak kurulum.aspx'e ekliyoruz
                    window.location.href = "kurulum.aspx?ids=" + idList;
                } else {
                    alert("Seçilen tarih için bir ID bulunamadı.");
                }
            }
...
<%--    <script>
    ...
    $(document).ready(function () {
        ...
        $.datepicker.setDefaults({
            ...
            dayNamesMin: ["Pz","Pt","Sa","Ça","Pe","Cu","Ct"],
            weekHeader: "Hf",
            dateFormat: "dd.mm.yy",  // Türk formatında tarih: Gün.Ay.Yıl
            firstDay: 1,  // Haftanın ilk günü Pazartesi olacak
            isRTL: false,
        ...
        $("#calendar").datepicker({
            onSelect: function (dateText, inst) {
                // Seçilen tarihi yyyy-mm-dd formatına çeviriyoruz
                var selectedDate = $.datepicker.parseDate("dd.mm.yy", dateText);
                var formattedDate = $.datepicker.formatDate("yy-mm-dd", selectedDate);

                // Veritabanından gelen tarihler ve ID'ler
                var id = calendarData[formattedDate]; // MSSQL'den gelen JSON verisi

                if (id) {
                    window.location.href = "kurulum.aspx?id=" + id;
                } else {
                    alert("Seçilen tarih için bir ID bulunamadı.");
                }
            }
